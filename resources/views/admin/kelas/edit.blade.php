<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-xl text-hitam leading-tight">{{ __('Edit Data Kelas') }}</h2>
    </x-slot>
                ...
                <div class="p-8 text-gray-900">
                    <form action="{{ route('admin.kelas.update', $kelas->id) }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-4">
                                <div>
                                    <x-input-label for="nama_kelas" :value="__('Nama Kelas')" class="text-hitam font-bold" />
                                    <x-text-input id="nama_kelas" class="block mt-1 w-full focus:ring-oranye focus:border-oranye" type="text" name="nama_kelas" value="{{ old('nama_kelas', $kelas->nama_kelas) }}" required />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <select name="instruktur_id" id="instruktur_id" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-oranye focus:ring-oranye" required>
                                        @foreach($instruktur as $ins)
                                            <option value="{{ $ins->id }}" {{ $kelas->instruktur_id == $ins->id ? 'selected' : '' }}>{{ $ins->user->name }} ({{ $ins->spesialisasi_las }})</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <x-text-input id="kuota_peserta" class="block mt-1 w-full focus:ring-oranye focus:border-oranye" type="number" name="kuota_peserta" value="{{ old('kuota_peserta', $kelas->kuota_peserta) }}" required />
                                </div>
                                <div>
                                    <x-input-label for="tanggal_mulai" :value="__('Tanggal Mulai')" class="text-hitam font-bold" />
                                    <x-text-input id="tanggal_mulai" class="block mt-1 w-full focus:ring-oranye focus:border-oranye" type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', $kelas->tanggal_mulai) }}" required />
                                </div>
                                <div>
                                    <x-input-label for="tanggal_selesai" :value="__('Tanggal Selesai')" class="text-hitam font-bold" />
                                    <x-text-input id="tanggal_selesai" class="block mt-1 w-full focus:ring-oranye focus:border-oranye" type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', $kelas->tanggal_selesai) }}" required />
                                </div>
                                <div>
                                    ...
                                    <select name="status_kelas" id="status_kelas" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-oranye focus:ring-oranye" required>
                                        <option value="menunggu" {{ $kelas->status_kelas == 'menunggu' ? 'selected' : '' }}>Menunggu</option>
                                        <option value="berjalan" {{ $kelas->status_kelas == 'berjalan' ? 'selected' : '' }}>Berjalan</option>
