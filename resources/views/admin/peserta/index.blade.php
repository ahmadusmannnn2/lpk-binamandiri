                        ...
                        <table class="min-w-full w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-gray-50 text-hitam uppercase text-xs font-black tracking-wider border-b-2 border-gray-200">
                                    ...
                                    <th class="py-4 px-6 text-left">Detail Kependudukan</th>
                                    <th class="py-4 px-6 text-left">Kontak & Berkas</th>
                                ...
                                @forelse($peserta as $item)
                                    <tr class="border-b border-gray-100 hover:bg-orange-50/30 transition duration-200">
                                        ...
                                        <td class="py-3 px-6">
                                            ...
                                            <div class="text-xs text-oranye font-semibold mt-0.5 mb-1">Daftar: {{ \Carbon\Carbon::parse($item->created_at)->format('d M Y') }}</div>
                                            @if($item->jenis_kelamin == 'Laki-laki')
                                                <span class="inline-block bg-blue-100 text-blue-800 text-[10px] px-2 py-0.5 rounded font-bold">Laki-laki</span>
                                            @elseif($item->jenis_kelamin == 'Perempuan')
                                                <span class="inline-block bg-pink-100 text-pink-800 text-[10px] px-2 py-0.5 rounded font-bold">Perempuan</span>
                                            @endif
                                        </td>
                                        
                                        <td class="py-3 px-6">
                                            <div class="font-bold text-gray-700">NIK: {{ $item->nik ?? '-' }}</div>
                                            <div class="text-xs text-gray-500 mt-1">
                                                Pendidikan: <span class="font-bold text-hitam">{{ $item->pendidikan_terakhir ?? '-' }}</span>
                                            </div>
                                        </td>
                                        
                                        <td class="py-3 px-6">
                                            <div class="font-bold text-gray-700 flex items-center gap-1 text-xs mb-1">
                                            ...
                                            <div class="flex gap-1 mt-1">
                                                @if($item->file_ktp) <span class="bg-green-100 text-green-700 text-[9px] px-1.5 py-0.5 rounded font-bold">KTP</span> @endif
                                                @if($item->file_ijazah) <span class="bg-green-100 text-green-700 text-[9px] px-1.5 py-0.5 rounded font-bold">Ijazah</span> @endif
                                            </div>
                                        </td>
                                        ...
                                        <td class="py-4 px-6">
                                            <div class="flex items-center justify-center gap-2 flex-wrap">
                                                <a href="{{ route('admin.peserta.edit', $item->id) }}"
                                                    class="bg-blue-50 text-blue-600 hover:bg-blue-500 hover:text-white border border-blue-200 px-2 py-1.5 rounded-md text-xs font-bold transition duration-300 flex items-center gap-1 shadow-sm">
                                                ...
                                                <form action="{{ route('admin.peserta.reset_password', $item->id) }}" method="POST"
                                                    ...
                                                    <button type="submit" title="Reset Sandi"
                                                        class="bg-yellow-50 text-yellow-600 hover:bg-yellow-500 hover:text-white border border-yellow-200 px-2 py-1.5 rounded-md text-xs font-bold transition duration-300 flex items-center gap-1 shadow-sm">
                                                ...
                                                <form action="{{ route('admin.peserta.destroy', $item->id) }}" method="POST"
                                                    ...
                                                    <button type="submit" title="Hapus Permanen"
                                                        class="bg-red-50 text-red-600 hover:bg-red-500 hover:text-white border border-red-200 px-2 py-1.5 rounded-md text-xs font-bold transition duration-300 flex items-center gap-1 shadow-sm">
