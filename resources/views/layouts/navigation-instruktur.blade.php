<nav x-data="{ open: false }" class="bg-hitam border-b border-oranye">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('instruktur.dashboard')"
                        :active="request()->routeIs('instruktur.dashboard')"
                        class="text-white hover:text-oranye focus:text-oranye {{ request()->routeIs('instruktur.dashboard') ? 'border-oranye text-oranye' : 'border-transparent' }}">
                        {{ __('Dashboard') }}
                    </x-nav-link>
                    <x-nav-link :href="route('instruktur.jadwal.index')"
                        :active="request()->routeIs('instruktur.jadwal.*')"
                        class="text-white hover:text-oranye focus:text-oranye {{ request()->routeIs('instruktur.jadwal.*') ? 'border-oranye text-oranye' : 'border-transparent' }}">
                        {{ __('Jadwal Mengajar & Nilai') }}
                    </x-nav-link>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            ...
                            <div>{{ Auth::user()->name }} (Instruktur)</div>
                            <div class="ms-1">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-hitam border-t border-oranye">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('instruktur.dashboard')"
                class="text-white hover:text-oranye">Dashboard</x-responsive-nav-link>
            <x-responsive-nav-link :href="route('instruktur.jadwal.index')"
                :active="request()->routeIs('instruktur.jadwal.*')" class="text-white hover:text-oranye">Jadwal Mengajar
