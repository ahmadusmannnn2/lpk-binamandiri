                                        ...
                                        @forelse($f->pertemuan as $index => $pertemuan)
                                            <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition duration-300 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 group">
                                                ...
                                                <div class="flex items-center gap-2 w-full sm:w-auto">
                                                    <div x-data="{ openKelolaMateri: false }" class="flex-1 sm:flex-none">
                                                        <button type="button" @click="openKelolaMateri = true" class="w-full text-center {{ $pertemuan->file_materi ? 'bg-blue-600 hover:bg-blue-500 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700' }} text-[11px] px-4 py-2 rounded-lg shadow font-bold transition flex items-center justify-center gap-1" title="{{ $pertemuan->file_materi ? 'Kelola Materi' : 'Tambah Materi' }}">
                                                        ...
                                                        <!-- Modal Kelola Materi -->
                                                        <div x-show="openKelolaMateri" style="display: none;" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                                                            <div @click.away="openKelolaMateri = false" class="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-sm relative whitespace-normal">
                                                                <h4 class="font-black text-hitam mb-4 text-lg border-b pb-2">Kelola Materi Pertemuan</h4>
                                                                
                                                                <div class="space-y-4">
                                                                    @if($pertemuan->file_materi)
                                                                        <!-- Tombol Lihat Materi -->
                                                                        <a href="{{ asset('storage/' . $pertemuan->file_materi) }}" target="_blank" class="w-full text-center bg-gray-100 hover:bg-gray-200 text-hitam px-4 py-2.5 rounded-lg shadow-sm font-bold transition flex items-center justify-center gap-2 text-sm">
                                                                            <svg class="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                                                            Lihat Materi Saat Ini
                                                                        </a>
                                                                    @else
                                                                        <div class="bg-yellow-50 text-yellow-700 p-3 rounded-lg text-xs font-bold border border-yellow-200 text-center">
                                                                            Belum ada materi untuk pertemuan ini.
                                                                        </div>
                                                                    @endif

                                                                    <!-- Form Update Materi -->
                                                                    <form action="{{ route('instruktur.pertemuan.update_materi', $pertemuan->id) }}" method="POST" enctype="multipart/form-data" class="bg-blue-50 p-3 rounded-xl border border-blue-100 mt-4 text-left">
                                                                        @csrf @method('PUT')
                                                                        <label class="block text-xs font-bold text-gray-700 mb-2">{{ $pertemuan->file_materi ? 'Ganti Materi Baru' : 'Upload Materi' }}</label>
                                                                        <input type="file" name="file_materi" accept=".pdf,.doc,.docx,.ppt,.pptx,.zip,.rar" class="block w-full text-[10px] text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:bg-blue-600 file:text-white hover:file:bg-blue-700 cursor-pointer mb-3" required />
                                                                        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow font-bold transition text-xs flex justify-center items-center gap-1">
                                                                    ...
                                                                    @if($pertemuan->file_materi)
                                                                        <!-- Form Hapus Materi -->
                                                                        <form action="{{ route('instruktur.pertemuan.destroy_materi', $pertemuan->id) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus materi ini?');" class="mt-2 text-left">
                                                                            @csrf @method('DELETE')
                                                                            <button type="submit" class="w-full bg-red-50 hover:bg-red-500 hover:text-white text-red-500 px-4 py-2 rounded-lg shadow-sm font-bold border border-red-200 transition text-xs flex items-center justify-center gap-2">
                                                                ...
                                                                <button type="button" @click="openKelolaMateri = false" class="mt-4 w-full px-4 py-2 text-xs font-bold text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 transition">Tutup</button>
                                                            </div>
                                                        </div>
                                                    </div>
